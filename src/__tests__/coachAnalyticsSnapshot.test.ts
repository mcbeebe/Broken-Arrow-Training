...
function isoWeekStartStr(): string {
  // Local components out, like the code under test (`localDateStr`). The old
  // `setHours(0,0,0,0)` + `toISOString()` read local MIDNIGHT in UTC, which is
  // the previous day at every positive offset — it only ever agreed with the
  // product because CI runs in UTC.
  const d = new Date()
  const day = d.getDay()
