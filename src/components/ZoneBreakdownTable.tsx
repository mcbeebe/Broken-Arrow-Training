...
export default function ZoneBreakdownTable({ heartrates, times, targetZone }: ZoneBreakdownTableProps) {
  ...
  return (
    <div className="mt-3 bg-slate-50 rounded-xl border border-slate-100">
      <button
        onClick={() => setExpanded(!expanded)}
        className="w-full flex items-center justify-between p-3"
      >
        <div className="text-left">
          <p className="text-xs font-semibold text-slate-700">Minute-by-Minute Zone Breakdown</p>
      ...
      {expanded && (
        <div className="px-1 pb-3">
          {/* Table header */}
          <div className="grid grid-cols-[40px_1fr_1fr_1fr_60px] gap-1 px-2 py-1 text-[10px] font-semibold text-slate-500 uppercase tracking-wide border-b border-slate-200">
            <span>Min</span>
          ...
          <div className="max-h-[400px] overflow-y-auto">
            {buckets.map((b) => {
              ...
              return (
                <div
                  key={b.minute}
                  className="grid grid-cols-[40px_1fr_1fr_1fr_60px] gap-1 px-2 py-1.5 text-[11px] border-b border-slate-100 last:border-0"
                >
                  <span className="text-slate-600 font-mono">{b.minute}</span>
                  <span className="text-right font-medium text-slate-700">{b.avgHR}</span>
                  <span className="text-right text-slate-500">{b.minHR}–{b.maxHR}</span>
          ...
          {/* Summary row */}
          <div className="grid grid-cols-[40px_1fr_1fr_1fr_60px] gap-1 px-2 py-2 text-[11px] font-bold border-t-2 border-slate-300 bg-slate-100 rounded-b-lg mt-1">
