...
import { ghostFillFromHistory, progressionFromWeeks, parsePlanExercises, CIRCUIT_DRAFT, type StrengthCalibration } from '../utils/strengthDraft'
...
export default function ManualLog({ dayLabel, existing, planned, allWeeks, strengthLevel, strengthCapacity, onSave, onClose, hasManualEntry, onRemove,
  ...
  const [exercises, setExercises] = useState<StrengthExerciseLog[]>(() => {
    ...
    if ((planned?.type === 'strength' || isGymCircuitDay) && planned?.detail) {
      return ghostFillFromHistory(parsePlanExercises(planned.detail, isGymCircuitDay ? CIRCUIT_DRAFT : undefined), progression, calibration)
    }
    return []
  })
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex gap-1.5">
                  {planned?.detail && (
                    <button
                      onClick={() => setExercises(ghostFillFromHistory(parsePlanExercises(planned.detail, isGymCircuitDay ? CIRCUIT_DRAFT : undefined), progression, calibration))}
              ...
              {pickerOpen && (
                <ExercisePicker
                  plannedExercises={planned?.detail ? parsePlanExercises(planned.detail, isGymCircuitDay ? CIRCUIT_DRAFT : undefined) : []}
