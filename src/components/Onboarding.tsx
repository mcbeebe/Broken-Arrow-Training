...
import {
  STEP_RACE_TYPE, STEP_RACE_NAME, STEP_EXPERIENCE, STEP_DETAIL, STEP_DAYS,
  STEP_VARIANT, STEP_BASELINE, STEP_EQUIPMENT, STEP_STRENGTH, STEP_SCHEDULE,
  STEP_WEARABLE, STEP_PROFILE, STEP_REVIEW, STEP_GENERAL_GOAL,
  STEP_GENERAL_CARDIO, STEP_MENOPAUSE, STEP_PREVIEW, STEP_GOAL_MODE,
  STEP_SEASON_RACES, STEP_HEALTH,
  stepName,
...
export default function Onboarding({ onComplete, onSkip, loadingDurationMs = 1800, previousConfig, derivedFitness, athleteId }: Props) {
  ...
  // The gates themselves — and why each one is drawn where it is — live in
  // onboarding/steps.ts, where they can be tested without driving the DOM.
  const showsMenopauseStep = gatesMenopauseStep({ age, sex })
  const visibleSteps: readonly number[] = computeVisibleSteps({
    raceType, goalMode, age, sex, hasProfilePrefill,
    previousExperienceLevel: prev?.experienceLevel,
  })
