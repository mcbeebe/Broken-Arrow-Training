...
SURFACE_INSTRUCTIONS = {
    "daily": (
        ...
        "RECONCILE THE CARDS — DO NOT INVENT A FOURTH VERDICT. The "
        "snapshot includes a `Signal coherence` line that compresses the "
        "three on-screen lenses — load (training math), body (today's "
        "biometrics), soreness (local muscular damage) — into a single "
        "call the athlete is already looking at. When coherence is "
        "'mixed' the cards disagree. Your job is to quote them by name "
        "in the athlete's language ('load says...', 'body says...', "
        "'soreness says...'), explain which one leads today and why, "
        "and end on the explicit call (rest / easy day / monitor / "
        "train as planned). When coherence is 'aligned' you can speak "
        "freely without enumerating each lens. Never reach a different "
        "conclusion than the `todayCall` field — the on-screen banner "
        "and your read must agree.\n"
