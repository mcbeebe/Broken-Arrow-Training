...
| `HEAT_PACE_ADJUST` (Phase 4) | easy/recovery bands ×1.015 (60–70°F) … ×1.09 (>90°F); quality effort-first ≥80°F | Direction from Ely et al. 2007 (T2 evidence); band multipliers are conservative translations | Calibration against athlete data; live-forecast source (v2) |
| Health-screen policy (Phase 4) | any yes: ramp ≤5%, plyo suppressed; bone stress: 6-wk impact hold; recent bone stress + marathon+: critical | IOC 2023 REDs consensus (screening-first, T1-adjacent); van der Worp 2015 (previous injury = strongest risk factor) | Clinical copy sign-off (named reviewer); threshold review |
